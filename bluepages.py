...
try:
    ...
    print "*"*24
    print "  PERSON INFORMATAION"
    print "*"*24
    print "CALLUPNAME          :",dict(r.json())["results"][0]["callupName"].encode("utf8")
    # print "RESULTLEVEL         :",dict(r.json())["results"][0]["resultLevel"]
    print "NAMEFULL            :",dict(r.json())["results"][0]["nameFull"]
    print "CO                  :",dict(r.json())["results"][0]["co"]
    print "UID                 :",dict(r.json())["results"][0]["uid"]
    if dict(r.json())["results"][0].has_key("telephone_office") :
        print "TELEPHONE_OFFICE    :",dict(r.json())["results"][0]["telephone_office"]
    else:
        print "TELEPHONE_OFFICE    : ****"

    print "ORG_TITLE           :",dict(r.json())["results"][0]["org_title"]
    print "DEPT_CODE           :",dict(r.json())["results"][0]["dept_code"]
    print "ID                  :",dict(r.json())["results"][0]["id"]
    print "PREFERREDIDENTITY   :",dict(r.json())["results"][0]["preferredIdentity"]
    # print "SCORE               :",dict(r.json())["results"][0]["score"]
    if dict(r.json())["results"][0].has_key("role") :
        print "ROLE                :",dict(r.json())["results"][0]["role"]
    else:
        print "ROLE                : ****"
        
    print "MAIL                :",dict(r.json())["results"][0]["mail"][0]
    print "HRORGANIZATIONCODE  :",dict(r.json())["results"][0]["hrOrganizationCode"]
    print "SERIAL              :",dict(r.json())["results"][0]["serial"]
    print "NOTESEMAIL          :",dict(r.json())["results"][0]["notesEmail"]
    ...
    print "*"*42
    print "  THE SAME TOWER MEMBERS(%s) - INCOUNTRY" % len(dict(rAll.json())["doc"]["content"]["incountry"]["peers"])
    print "*"*42
    ...
    print "*"*36
    print "  REPORT MANAGERS(%s) - INCOUNTRY" % len(dict(rAll.json())["doc"]["content"]["incountry"]["leadership"])
    print "*"*36
    ...
    '''
    print "*"*25
    print "  The Same Tower Member - functional"
    print "*"*25
    for detail in dict(rAll.json())["doc"]["content"]["functional"]["peers"]:
        print detail["uid"] + " -", \
        detail["name"]["first"].encode("utf8") + " " +detail["name"]["last"].encode("utf8")  + " -",\
        detail["preferredIdentity"] 

    
    print "*"*20
    print "  Report Managers - functional"
    print "*"*20
    for detail in dict(rAll.json())["doc"]["content"]["functional"]["leadership"]:
        print detail["uid"] + " -", \
        detail["name"]["first"].encode("utf8") + " " +detail["name"]["last"].encode("utf8")  + " -",\
        detail["preferredIdentity"] + " -",\
        detail["role"] 
